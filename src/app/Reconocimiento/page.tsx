...
export default function ReconocimientoPage() {
  ...
  const actualizarArea = <K extends keyof typeof areas[number]>(
    index: number,
    key: K,
    data: SetStateAction<typeof areas[number][K]>
  ) => {
    setAreas((prevAreas) => {
      const newAreas = [...prevAreas];
      newAreas[index] = {
        ...newAreas[index],
        [key]: typeof data === 'function' ? data(newAreas[index][key]) : data,
      };
      return newAreas;
    });
  };
